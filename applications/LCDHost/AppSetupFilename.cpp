...
AppSetupFilename::AppSetupFilename(QWidget *parent) : QWidget(parent)
{
    ...
    connect( tb_, SIGNAL(clicked()), this, SLOT(selectFile()) );
    connect( le_, SIGNAL(textEdited(QString)), this, SIGNAL(textEdited(QString)) );
    setLayout(layout);
    return;
}

void AppSetupFilename::setEnabled(bool b)
{
    le_->setEnabled(b);
    tb_->setEnabled(b);
    return;
...
void AppSetupFilename::setText(QString s)
{
    if( QFileInfo(s).exists() ) le_->setText( QDir( mainWindow->layoutPath ).relativeFilePath( s ) );
    else le_->setText( s );
    return;
}

void AppSetupFilename::selectFile()
{
    if( mainWindow )
    {
