...
void CPlayerFoobar::Initialize()
{
	...
	if (m_FooWindow)
	{
		...
		if (version < 100)
		{
                    qWarning() << "LH_NowPlaying: Your copy of the foo_rainmeter.dll plugin for foobar2000 is outdated.";
                    qWarning() << "LH_NowPlaying: Download the latest version from foo-rainmeter.googlecode.com and try again.";
                    m_FooWindow = NULL;
		}
		else
		{
